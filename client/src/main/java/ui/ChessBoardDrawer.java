...
public class ChessBoardDrawer {
    ...
    public void printWithHighlights(ChessPosition from, Collection<ChessMove> legalMoves) {
        ...
        int[] rowRange = whitePerspective ? range(8, 1, -1) : range(1, 8, 1);
        for (int row : rowRange) {
            printRowWithHighlights(row, whitePerspective, highlights, from);
        }
    ...
    private void printRowWithHighlights(int row, boolean whitePerspective, Set<ChessPosition> highlights, ChessPosition from) {
        System.out.print(SET_BG_COLOR_BLACK + SET_TEXT_COLOR_BLUE + " " + row + " " + RESET_TEXT_COLOR);
        int[] colRange = whitePerspective ? range(1, 8, 1) : range(8, 1, -1);

        for (int col : colRange) {
            ChessPosition pos = new ChessPosition(row, col);
            boolean isFrom = pos.equals(from);
            boolean isHighlight = highlights.contains(pos);

            printHighlightedSquare(pos, isFrom, isHighlight);
        }

        System.out.println(SET_BG_COLOR_BLACK + SET_TEXT_COLOR_BLUE + " " + row + " " + RESET_BG_COLOR + RESET_TEXT_COLOR);
    }

    private void printHighlightedSquare(ChessPosition pos, boolean isFrom, boolean isHighlight) {
        int row = pos.getRow();
        ...
        String bgColor;
        if (isFrom) {
            bgColor = SET_BG_COLOR_YELLOW; // origin square
        } else if (isHighlight) {
            bgColor = SET_BG_COLOR_GREEN; // legal destination square
        } else {
            bgColor = ((row + col) % 2 == 0) ? SET_BG_COLOR_LIGHT_GREY : SET_BG_COLOR_RED;
        }
